    ...
    public static void main(String[] args) {
        ...
        try (Connection conn = DriverManager.getConnection(URL, USER, PASSWORD);
            ...
            try (ResultSet rs = stmt.executeQuery(
                    "SELECT d.Jour, d.Developpeurs, d.NbScripts AS MaxScripts " +
                            "FROM DevData d " +
                            "INNER JOIN (SELECT Jour, MAX(NbScripts) AS MaxScripts FROM DevData GROUP BY Jour) m " +
                            "ON d.Jour = m.Jour AND d.NbScripts = m.MaxScripts"
            )) {
                while (rs.next()) {
